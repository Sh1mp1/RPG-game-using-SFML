...
void SpiderEnemy::initComponents(sf::Texture& texture_sheet)
{
	this->initMovementComponent(150.f, 0.f, 9999, 1100, true);
...
void SpiderEnemy::initAnimations(sf::Texture& texture_sheet)
{
}

SpiderEnemy::SpiderEnemy(const float pos_x, const float pos_y, EnemySpawner& enemy_spawner)
	:spiderTexture(), Enemy(pos_x, pos_y, this->spiderTexture, enemy_spawner)
{
	if (!this->spiderTexture.loadFromFile("Textures/spider1_60x64.png"))
	{
...
void SpiderEnemy::updateAnimation(const float& dt)
{
...
void SpiderEnemy::render(sf::RenderTarget& target, sf::Shader* shader, const bool show_hitbox, sf::Vector2f lightPos)
{
	if (shader)
	{
